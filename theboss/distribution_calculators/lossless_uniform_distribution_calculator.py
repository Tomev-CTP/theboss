...
"""
    ...
    uniform sampler from the lossless BS distribution. 
"""
...
class LosslessUniformBSDistributionCalculator(BSDistributionCalculatorInterface):
    """
    A class for computing the output probabilities of the lossless uniform sampling
    from BS distribution with a fixed number of particles and modes.
    """
    ...
    def calculate_distribution(self) -> List[float]:
        ...
        return self.calculate_probabilities_of_outcomes(
            generate_possible_states(self._particles_number, self._modes_number)
        )
    ...
    def get_outcomes_in_proper_order(self) -> List[Sequence[int]]:
        ...
        """
        return generate_possible_states(self._particles_number, self._modes_number)
