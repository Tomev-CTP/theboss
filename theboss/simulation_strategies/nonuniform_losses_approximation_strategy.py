...
"""
    This file contains implementation of approximate boson sampling strategy subject to
    non-uniform losses. This can be well used to approximate boson sampling experiments
    with non-balanced network. More details can be found in [2].
"""
...
class NonuniformLossesApproximationStrategy:
    ...
    def _compute_lossy_input(self, input_state: ndarray) -> ndarray:
        ...
        lossy_input[binned_input_index] = choice(
            range(self._approximated_modes_number + 1), p=self._binomial_weights
        )
